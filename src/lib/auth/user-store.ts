...
export async function authenticateGoogleUser(input: {
  ...
  const existingUser = await findWordPressUserByEmail(normalizedEmail);
  ...
  const displayName = input.name?.trim() || normalizedEmail.split("@")[0] || "Usuario";

  const created = await createWordPressUser({
    name: displayName,
  ...
  // Race condition: account may have been created between checks
  const fetched = await findWordPressUserByEmail(normalizedEmail);
  if (fetched) {
    return { ok: true, user: fetched };
  }

  throw new Error("No pudimos completar el acceso con Google.");
}
