        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="space-y-2">
            ...
            <p className="max-w-2xl text-sm leading-7 text-black/60">
              Regístrate como vendedor, publica tus productos y llega a la comunidad
              hispana en Serbia. Solo cobramos una comisión del 10% por venta.
            </p>
          </div>
          <Link
            href="/vender"
...
function ProductCard({ product }: { product: StoreProduct }) {
  ...
  // Only show stock badge when there's a reason: out of stock or low stock
          ...
          <div className="flex items-center gap-1.5">
            <span
              className={`h-1.5 w-1.5 shrink-0 rounded-full ${
                outOfStock ? "bg-red-400" : "bg-amber-400"
              }`}
            />
            <span
              className={`text-[0.6rem] font-medium ${
                outOfStock ? "text-red-500" : "text-amber-600"
              }`}
            >
              {stockLabel}
            </span>
